class PaymentsController < ApplicationController
  ...
  def success
    ...
    if paid_status && @booking.update(reservation_status: paid_status)
      ...
      redirect_to property_booking_path(@booking.property, @booking), notice: 'Payment successful! Your booking is confirmed.'
    else
      Rails.logger.error "Failed to update booking: #{@booking.errors.full_messages}"
      redirect_to property_booking_path(@booking.property, @booking), alert: 'Payment was successful, but there was an issue updating the booking.'
    end
  end
